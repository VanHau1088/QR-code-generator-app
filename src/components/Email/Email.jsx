...
// Màu chấm góc 
  ...
  }, [email, subject, message, dotType, dotColor, bgColor, cornerSquareType, cornerDotType, bgSquareType, bgDotType, logo]);

 // qr dynamic 
 useEffect(() => { 
  if (email && subject && message && !isCreatingQRCode) { 
    setIsCreatingQRCode(true); 
    createDynamicQRCode('email', { email, subject, message }); 
  } }, [email, subject, message]);

  useEffect(() => { 
    createDynamicQRCode('email', { email, subject, message }); 
  }, [email, subject, message, dotType, dotColor, bgColor, cornerSquareType, bgSquareType, cornerDotType, bgDotType, logo]);
...
const saveQRCodeToDatabase = async ( type, data, userId) => {
    ...
    const handleDownloadClick = async () => {
      try{
        await saveQRCodeToDatabase('email', {email, subject, message}, userData._id);
